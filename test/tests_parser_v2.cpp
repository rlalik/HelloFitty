...
TEST(TestsParserV2, Parsing1Function)
{
    ...
    ASSERT_EQ(hfp->get_functions_count(), 1);
    ASSERT_STREQ(hfp->get_function(0), "gaus(0)");

    ASSERT_EQ(hfp->get_function_params_count(), 3);

    ASSERT_EQ(hfp->get_param(0).value, 1);
    ASSERT_EQ(hfp->get_param(1).value, 2);
    ASSERT_EQ(hfp->get_param(2).value, 3);
}
...
TEST(TestsParserV2, Parsing2Functions)
{
    ...
    ASSERT_EQ(hfp->get_functions_count(), 2);
    ASSERT_STREQ(hfp->get_function(0), "gaus(0)");
    ASSERT_STREQ(hfp->get_function(1), "expo(3)");

    ASSERT_EQ(hfp->get_function_params_count(), 5);

    ASSERT_EQ(hfp->get_param(0).value, 1);
    ASSERT_EQ(hfp->get_param(1).value, 2);
    ASSERT_EQ(hfp->get_param(2).value, 3);

    ASSERT_EQ(hfp->get_param(3).value, 4);
    ASSERT_EQ(hfp->get_param(4).value, 0);
}
...
TEST(TestsParserV2, Parsing3Functions)
{
    ...
    ASSERT_EQ(hfp->get_functions_count(), 3);
    ASSERT_STREQ(hfp->get_function(0), "gaus(0)");
    ASSERT_STREQ(hfp->get_function(1), "expo(3)");
    ASSERT_STREQ(hfp->get_function(2), "pol1(5)");

    ASSERT_EQ(hfp->get_function_params_count(), 7);
    ...
    ASSERT_EQ(hfp->get_param(3).value, 4);
    ASSERT_EQ(hfp->get_param(4).value, 5);

    ASSERT_EQ(hfp->get_param(5).value, 6);
    ASSERT_EQ(hfp->get_param(6).value, 0);
}
...
TEST(TestsParserV2, Formatting3Functions)
{
    ...
    auto out = hf::tools::format_line_entry(hfp.get(), hf::format_version::v2);
...
TEST(TestsParserV2, ComplexLine)
{
    ...
    ASSERT_EQ(hfp->get_functions_count(), 3);
    ASSERT_EQ(hfp->get_function_params_count(), 5);
    ASSERT_STREQ(hfp->get_function(0), "[0]+[1]*x");
    ASSERT_STREQ(hfp->get_function(1), "[0]+[4]*x");
    ASSERT_STREQ(hfp->get_function(2), "[1]+[2]*x");
